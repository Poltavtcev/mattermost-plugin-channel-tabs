...
// NOTE: We intentionally embed the manifest as a JS object literal (not a JSON string)
// to avoid runtime JSON.parse issues with escaped quotes/backslashes inside strings.
const manifest = %s;
...
func applyManifest(manifest *model.Manifest) error {
	...
	if manifest.HasWebapp() {
		// Generate a JSON representation of Manifest.
		// JSON is compatible with JS object literals, so we embed it directly.
		manifestBytes, err := json.MarshalIndent(manifest, "", "    ")
		if err != nil {
			return err
		}
